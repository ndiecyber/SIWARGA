...
function FilterDropdown<TData>({
  ...
  const [hoveredCategory, setHoveredCategory] =
    useState<FilterCategory<TData> | null>(null);
  ...
  const handleCategoryHover = (cat: FilterCategory<TData>) => {
    setHoveredCategory(cat);

    const el = categoryRefs.current.get(String(cat.id));
    if (el) {
          ...
          <div className="w-44 rounded-r-md border bg-popover py-1">
            ...
            {filterCategories.map((cat) => (
              <div
                key={String(cat.id)}
                ref={(el) => {
                  if (el) categoryRefs.current.set(String(cat.id), el);
                }}
                className={cn(
